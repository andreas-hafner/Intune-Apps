...
.SYNOPSIS
Install es-ES Language Pack over Intune, tested with Autopilot Pre-Provisioning process.
	
.DESCRIPTION
Install es-ES Language Pack over Intune, tested with Autopilot Pre-Provisioning process. Should work since 2004 / 20H1, tested for Pre-Provisioning with Windows 10 21H2.
...
Adjust this script and the script update-source.ps1 for your needs and run update-source.ps1 to gather the required files from the source medias

.NOTES
Author     : Fabian Niesen (www.fabian-niesen.de)
Filename   : Win10-21H2-LanguagePack-es-ES.ps1
Requires   : PowerShell Version 4.0
Version    : 1.0.0
History    : 1.0.0   FN  26.08.2022  Initial Version
...
$transfile = $logPath + "\$LogName-Transscript.txt"
Start-Transcript -Path $transfile 
Add-Type -AssemblyName Microsoft.VisualBasic
$script:EventLogName = "Application"
$script:EventLogSource = "EventSystem"
    if ($VerbosePreference -eq 'Continue') {Start-Transcript -Path "$script:logPath\Transcript.log" -Append }
    ####################################################
    ####################################################
    #Build Functions
    ####################################################
    ...
    If ($Install) {
        ...
        Write-Log -message "Detected Windows Buildnumber: $OSbuildnumber"
        IF ( $OSbuildnumber -lt 22000 ) 
        {
            ...
            ### Based upon https://docs.microsoft.com/en-us/azure/virtual-desktop/windows-11-language-packs
            Write-Log -Message "Disable Language Pack Cleanup"
            $taskpath = "\Microsoft\Windows\AppxDeploymentClient\"
            $taskname = $($(Get-ScheduledTask -TaskPath $taskpath -ErrorAction Ignore ).TaskName)
            IF ($taskname -ne $null)
            {
                Disable-ScheduledTask -TaskPath $taskpath -TaskName $($(Get-ScheduledTask -TaskPath $taskpath).TaskName)
                IF ( $(Get-ScheduledTask -TaskPath $taskpath).State -eq "Disabled" )
                { Write-log -message "Task $taskpath is disabled" } Else { Write-log  -message "Task $taskpath is NOT disabled" -LogLevel 3 }
            }
            ...
            $cabs = (get-childitem -Recurse -Attributes !directory -Filter *.cab )
            Write-Log -message "Found $($cabs.count) CAB file for inserting"
            Foreach ( $cab in $cabs)
            {
                Add-WindowsCapability -Online -name $cab.BaseName -Source $cab.directory
            }
            $LXPS =  (get-childitem -Recurse -Attributes !directory -Filter *.appx )
            IF ($LXPS.Count -gt 0) {
            Write-Log -message "Add $($LXPS.count) LXP Files"
            Foreach ( $LXP in $LXPS)
            {
                Add-AppxProvisionedPackage  -Online -PackagePath $LXP.FullName -LicensePath $($LXP.Directory.FullName + "\License.xml")
                Add-Appx
            }
            } Else { Write-log -message "No LXPS found"}

            Write-Log -message "Following settings will be overwritten by OOBE if Autopilotprofile setting >Language (Region)< is set to >User select<"
            $LanguageList = Get-WinUserLanguageList
            Write-Log -message "Detected language list: `n$LanguageList"
            $LanguageList.Add($langname)
            $LanguageList = $LanguageList[-1..$($LanguageList.count -2)]
            Write-Log -Message "Set WinUserLanguageList - Primary: $($LanguageList[0].LanguageTag)"
            Set-WinUserLanguageList $LanguageList[0] -force -ErrorAction Continue
            Write-Log -Message "LanguageList: $($(Get-WinUserLanguageList).LanguageTag)"
            #Set-Culture only works for the active User!
            Write-Log -Message "Set Culture to $langname"
            Set-Culture -CultureInfo $langname
            If ( $($(Get-Culture).Name) -eq $langname ) {Write-Log -Message "Culture: $($(Get-Culture).Name)"} else {Write-Log -Message "Culture: $($(Get-Culture).Name)" -LogLevel 3}
            Write-Log -Message "Set TimeZone to $timezone"
            Set-TimeZone -Name $timezone
            If ( $($(Get-TimeZone).Id) -eq $timezone ) {Write-Log -Message "TimeZone $($(Get-TimeZone).Id)"} else {Write-Log -Message "TimeZone $($(Get-TimeZone).Id)" -LogLevel 3}

            Write-Log -Message "Set WinSystemLocale to $langname"
            Set-WinSystemLocale -SystemLocale $langname -Verbose
            If ( $($(Get-WinSystemLocale).Name) -eq $langname ) {Write-Log -Message "SystemLocale: $($(Get-WinSystemLocale).Name)"} else {Write-Log -Message "Reboot is required to change SystemLocale" -LogLevel 2}
            Write-Log -Message "Copy-UserInternationalSettingsToSystem"
            Copy-UserInternationalSettingsToSystem -WelcomeScreen $True -NewUser $True -verbose -ErrorAction Ignore
        ...
        Else {Write-log -message "This is Windows 11 - Skip execution" -LogLevel 2}
    }
    ElseIf ( $UnInstall ) {
