...
If ($Install) {
    ...
    Update-Help -SourcePath .\PS-Help -Recurse -Confirm:$false
    ...
    If (! ($userInstall) ) {
            ...
            If ( $regTag ) {
                Write-Log -Message "Using RegTag: HKEY_LOCAL_MACHINE\Microsoft$ScriptName"
...
ElseIf ( $UnInstall ) {
    Write-Log -Message "No Uninstall steps available! Sorry" -LogLevel 2
